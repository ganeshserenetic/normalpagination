...
class PdoOpt {
  ...
  public function initiateConnection() {
    ...
    try {
      if (!isset($db['port'])) {
        ...
        $this->dbh = new PDO("mysql:host={$host};port={$db['port']};dbname={$dbname}", $username, $password);
      }
      $this->displayError();
    } catch (Exception $e) {
      $tmp_arr = array();
  ...
  protected function errorLog($message) {
    ...
    if (in_array($_SERVER['HTTP_HOST'], $LOCAL_IP)) {
      echo "<pre>";
      print_R($messageArr);
      echo "</pre>";
      exit();
    ...
    //check number of errors sent in last minute
    // if ($this->checkNumErrors()) {
    //  // exit('Something went wrong...');
    // }
    //log error time to see how many errors sent on email in last minute
    //$this->appendErrorLog();

    $mail= new cyberxMail();
    $mail->setFrom("info@nucleogenex.com");
    $mail->setSubject('Uforia: SQL Error occur in ' . $SITE_ENV);
    $mail->addTo('punit@cyberxllc.com');
    $mail->addTo('suresh@cyberxllc.com');
    $mail->addTo('sqlnotify@cyberxllc.com');
    $mail->addContent("text/html","<pre>" . print_r($messageArr, true) . "</pre>");
    $mail->sendMail();
    exit('Something went wrong.');    
